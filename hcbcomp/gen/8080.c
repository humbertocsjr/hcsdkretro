...
// 8080 code generator uses SP-relative frame pointer
// Primary = HL, Secondary = DE
...
void gen_prologue(const char *name, int nlocals)
{
    ...
    if (nlocals > 0)
    {
        gen_emitf("lxi h, %i", (nlocals * -2) & 0xFFFF);
        gen_emit("dad sp");
        gen_emit("sphl");
    }
}

void gen_epilogue(void) { gen_emit("ret"); }
