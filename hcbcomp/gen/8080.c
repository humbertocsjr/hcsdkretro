...
// [English] Generates a new unique label number for 8080 assembly
// [Portuguese] Gera um novo número de rótulo único para assembly 8080
...
// [English] Emits a string as a DB directive with escape sequence handling
// [Portuguese] Emite uma string como diretiva DB com tratamento de sequências de escape
...
// [English] Emits a formatted comment line in 8080 assembly
// [Portuguese] Emite uma linha de comentário formatada em assembly 8080
...
// [English] Generates function prologue for 8080: saves BC (frame pointer),
// sets up BC = SP+2, allocates locals initialized to zero
// [Portuguese] Gera prólogo de função para 8080: salva BC (ponteiro de quadro),
// configura BC = SP+2, aloca locais inicializados com zero
...
// [English] Generates function epilogue for 8080: restores SP, pops BC, returns
// [Portuguese] Gera epílogo de função para 8080: restaura SP, desempilha BC, retorna
...
// [English] Generates return (same as epilogue for 8080)
// [Portuguese] Gera retorno (mesmo que epílogo para 8080)
...
// [English] Stores HL to a local variable at given offset from BC (frame pointer)
// [Portuguese] Armazena HL em uma variável local no deslocamento fornecido de BC (ponteiro de quadro)
...
// [Portuguese] Computa endereço de uma variável local em HL usando BC+offset
...
// [Portuguese] Computa endereço de um parâmetro em HL usando BC+offset
...
// [English] 16-bit subtraction for 8080: HL = DE - HL
// [Portuguese] Subtração de 16 bits para 8080: HL = DE - HL
void gen_sub(void)
{
    gen_emit("mov a, e");
    gen_emit("sub l");
...
// [English] 16-bit right shift: HL = DE >> HL
// [Portuguese] Deslocamento à direita de 16 bits: HL = DE >> HL
void gen_shr(void)
{
    int l_done = gen_label();
    int l_loop = gen_label();
...
// [English] Generates function call and adjusts stack for arguments on 8080
// [Portuguese] Gera chamada de função e ajusta a pilha para argumentos no 8080
...
// [English] 8080 Pattern 1: replaces "push h; pop b" with "mov b,h; mov c,l" (faster).
// [Portuguese] Padrão 8080 1: substitui "push h; pop b" por "mov b,h; mov c,l" (mais rápido).
static int i80_match_push_pop_b(peep_line_t *w, peep_line_t *repl)
{
...
// [English] 8080 Pattern 2: replaces "push h; lxi h,N; pop d; xchg" with "xchg; lxi h,N; xchg".
// [Portuguese] Padrão 8080 2: substitui "push h; lxi h,N; pop d; xchg" por "xchg; lxi h,N; xchg".
static int i80_match_push_lxi_pop_xchg(peep_line_t *w, peep_line_t *repl)
{
    if (!peep_op_args(&w[0], "push", 1) || strcmp(w[0].args[0], "h")) return 0;
...
// [English] 8080 Pattern 3: replaces "xchg; lxi h,IMM; xchg; mov a,e; mov m,a; inx h; mov a,d; mov m,a"
// with optimized immediate store using MVI instead of LXI+XCHG
// [Portuguese] Padrão 8080 3: otimiza store de imediato em variável local usando MVI
static int i80_match_imm_store(peep_line_t *w, peep_line_t *repl)
{
...
{
    if (wcount == 8) {
        int n = i80_match_imm_store(window, repl);
        if (n > 0) return n;
    }
    if (wcount == 4) {
        int n = i80_match_push_lxi_pop_xchg(window, repl);
        if (n > 0) return n;
    }
    if (wcount == 2) {
        int n = i80_match_push_pop_b(window, repl);
        if (n > 0) return n;
    }
    return 0;
}
