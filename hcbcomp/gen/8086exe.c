...
void gen_reverse_args(int count)
{
	...
	case 3:
		...
		break;
	case 4:
		gen_emit("; reverse 4 far args");
		gen_emit("pop bx");
		gen_emit("pop cx");
		gen_emit("pop si");
		gen_emit("pop di");
		gen_emit("xchg sp, bp");
		gen_emit("xchg di, [bp-8]");
		gen_emit("xchg si, [bp-10]");
		gen_emit("xchg cx, [bp-12]");
		gen_emit("xchg bx, [bp-14]");
		gen_emit("xchg sp, bp");
		break;
	default:
		gen_emitf("; reverse %i far args (unimplemented)", count);
		break;
	}
}
