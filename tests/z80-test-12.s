...
LOOP_F:
    ...
    ; =========================================================
    ; PASSO 1: O Gabarito Perfeito para CCF
    ; =========================================================
    
    ; --- Expected A ---
    LD A, D
    LD [EXP_A_VAR], A   ; O A não é alterado pelo CCF
    ...
    ; Regra de Ouro do CCF: A nova flag H é igual à antiga flag C
    LD A, E             ; Pega o Old F
    AND 0x01            ; Isola a antiga Carry
    RLCA
    RLCA
    RLCA
    RLCA ; Move o bit 0 para o bit 4 (Posição do H)
    OR L
    LD L, A
    
    ; Inverte a Carry
    LD A, E
    XOR 0x01            ; Inverte o bit 0
    AND 0x01            ; Garante que pegamos só ele
    OR L
    LD L, A
