...
void help()
{
    printf("HC Linker for Retro Computing [%s]\n", format_name());
...
int main(int argc, char **argv)
{
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        {
            i++;
            if(sym_name) error("error: symbol file name already defined\n");
            if(i < argc) sym_name = argv[i];
        }
    ...
    if(!out_name) out_name = format_default_out_name();
    if(!_objs) help();
    format_init();
    out_open(out_name);
    ...
    out_close();
    if(sym_name) consts_print(sym_name);
    return 0;
}
